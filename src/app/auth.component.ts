...
export class AuthComponent {
  ...
  private captureLocationOnLogin() {
    // request a one-time position and then start a watch to keep location updated in localStorage
    if (!navigator.geolocation) {
      this.toast.show('Geolocation not supported in this browser', 'warning');
      return;
    }
    navigator.geolocation.getCurrentPosition((pos) => {
      const payload = { lat: pos.coords.latitude, lon: pos.coords.longitude, updatedAt: (new Date()).toISOString() };
      try { localStorage.setItem('passenger_location', JSON.stringify(payload)); } catch {}
      // attempt to POST to backend if we have an authenticated passenger id
      const me = this.auth.current;
      if (me && me.id) {
        try { this.data.postPassengerLocation(me.id, payload.lat, payload.lon).subscribe(() => {}, () => {}); } catch {}
      }
      // start a watch to keep updating passenger location
      try {
        const watchId = navigator.geolocation.watchPosition((p) => {
          const pl = { lat: p.coords.latitude, lon: p.coords.longitude, updatedAt: (new Date()).toISOString() };
          localStorage.setItem('passenger_location', JSON.stringify(pl));
          const m = this.auth.current;
          if (m && m.id) {
            try { this.data.postPassengerLocation(m.id, pl.lat, pl.lon).subscribe(() => {}, () => {}); } catch {}
          }
        }, (err) => { /* ignore watch errors */ }, { enableHighAccuracy: true, maximumAge: 2000 });
        localStorage.setItem('passenger_location_watch', String(watchId));
      } catch (e) {
        // ignore
      }
    }, (err) => {
      this.toast.show('Location permission denied — unable to enable live tracking', 'warning');
    }, { enableHighAccuracy: true, timeout: 10000 });
  }

  private captureOwnerLocationOnLogin() {
    if (!navigator.geolocation) {
      this.toast.show('Geolocation not supported in this browser', 'warning');
      return;
    }
    navigator.geolocation.getCurrentPosition((pos) => {
      const payload = { lat: pos.coords.latitude, lon: pos.coords.longitude, updatedAt: (new Date()).toISOString() };
      try { localStorage.setItem('owner_location', JSON.stringify(payload)); } catch {}
      const me = this.auth.current;
      const ownerId = (me && (me as any).ownerId) ? (me as any).ownerId : (me ? me.id : undefined);
      if (ownerId) {
        try { this.data.postOwnerLocation(ownerId, payload.lat, payload.lon).subscribe(() => {}, () => {}); } catch {}
      }
      try {
        const watchId = navigator.geolocation.watchPosition((p) => {
          const pl = { lat: p.coords.latitude, lon: p.coords.longitude, updatedAt: (new Date()).toISOString() };
          localStorage.setItem('owner_location', JSON.stringify(pl));
          const m = this.auth.current;
          const oid = (m && (m as any).ownerId) ? (m as any).ownerId : (m ? m.id : undefined);
          if (oid) {
            try { this.data.postOwnerLocation(oid, pl.lat, pl.lon).subscribe(() => {}, () => {}); } catch {}
          }
        }, (err) => { /* ignore */ }, { enableHighAccuracy: true, maximumAge: 1000 });
        localStorage.setItem('owner_location_watch', String(watchId));
      } catch (e) {
        // ignore
      }
    }, (err) => {
      this.toast.show('Location permission denied — unable to enable live tracking for owner', 'warning');
    }, { enableHighAccuracy: true, timeout: 10000 });
  }
